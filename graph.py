...
def create_bar(Category: str, axes: plt.axes, ax: int):
    # Counting Data
    ...
    axes[ax].bar(x - w/2, output_counts, width=w, label='IA')
    axes[ax].bar(x + w/2, input_counts, width=w, label='Human')
    
    # Labels
    axes[ax].set_xticks(x)
    axes[ax].set_xticklabels(keys, rotation=45, ha="right")
    axes[ax].set_ylabel(Category)
    axes[ax].set_title(Category + " - Comparison")
